...
{
    public class AvailabilitySlotService(IAvailabilitySlotRepository availabilitySlotRepository) : IAvailabilitySlotService
    {

        public async Task<IEnumerable<AvailabilitySlotDTO>> GetByDayAsync(DayOfWeek day) 
            => await availabilitySlotRepository.GetQueryable()
        ...
        public async Task<Result<IEnumerable<AvailabilitySlotDTO>>> GetByDoctorIdAsync(int doctorId)
        {
            ...
            var doctorExists = await availabilitySlotRepository.GetQueryable()
            ...
            var availabilitySlot =  await availabilitySlotRepository.GetQueryable()
        ...
        public async Task<Result<CreateAvailabilitySlotDTO>> CreateAvailabilitySlotAsync(CreateAvailabilitySlotDTO dto) {
            ...
            var timeOver = await availabilitySlotRepository.GetQueryable().AnyAsync(x =>
            ...
            if(timeOver)
                return Result<CreateAvailabilitySlotDTO>.Failure("The specified time slot overlaps with an existing availability slot for the doctor.", HttpStatusCode.Conflict);
            ...
            return Result<CreateAvailabilitySlotDTO>.Success(createdDto ,"Availability slot created successfully.", HttpStatusCode.Created);
        }
